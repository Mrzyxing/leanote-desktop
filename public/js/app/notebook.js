...
Notebook.getSimpleTreeSetting = function (options) {
  ...
  function addDiyDom(treeId, treeNode) {
    ...
    if (treeNode.level > 1) {
      var spaceStr = "<span style='display: inline-block;width:" + (spaceWidth
...
Notebook.getTreeSetting = function (isSearch, isShare) {
  ...
  function addDiyDom(treeId, treeNode) {
    ...
    if (!Notebook.isAllNotebookId(treeNode.NotebookId)
        && !Notebook.isTrashNotebookId(treeNode.NotebookId)) {
      icoObj.after($('<span class="notebook-number-notes" id="numberNotes_'
          + treeNode.NotebookId + '">' + (treeNode.NumberNotes || 0)
          + '</span>'));
      // icoObj.after($('<span class="notebook-dirty" id="notebookDirty_' + treeNode.NotebookId + '"></span>'));
      icoObj.after(
          $('<span class="fa notebook-setting" title="' + getMsg('Setting')
              + '"></span>'));
    }
  ...
  function onDrop(e, treeId, treeNodes, targetNode, moveType) {
    ...
    for (var i in nodes) {
      ...
      if (!Notebook.isAllNotebookId(notebookId) && !Notebook.isTrashNotebookId(
              notebookId)) {
    ...
    NotebookService.dragNotebooks(ajaxData.curNotebookId,
        ajaxData.parentNotebookId, ajaxData.siblings);
          ...
          if (childNotes) {
            childNotes.forEach(function (node) {
              ...
              if (Notebook._subNotebookNumberNotes[notebookId] !== undefined) {
                $('#numberNotes_' + notebookId).html(
                    Notebook._subNotebookNumberNotes[notebookId]);
...
Notebook.renderNotebooks = function (notebooks, reload) {
  ...
  self.tree = $.fn.zTree.init($("#notebookList"), self.getTreeSetting(),
      notebooks);
...
Notebook.searchNotebookForAddNote = function (key) {
  ...
  if (key) {
    ...
    for (var i in notebooks) {
      ...
      if (!self.isAllNotebookId(notebookId) && !self.isTrashNotebookId(
              notebookId)) {
...
Notebook.renderShareNotebooks = function (sharedUserInfos, shareNotebooks) {
  ...
  if (!shareNotebooks || typeof shareNotebooks != "object"
      || shareNotebooks.length < 0) {
    return;
  }
  ...
  for (var i in sharedUserInfos) {
    ...
    var userNotebooks = user2ShareNotebooks[userInfo.UserId]
        || {ShareNotebooks: []};

    userNotebooks.ShareNotebooks = [{NotebookId: "-2", Title: "默认共享"}].concat(
        userNotebooks.ShareNotebooks)
    ...
    for (var j in userNotebooks.ShareNotebooks) {
      ...
      body += tt('<li><a notebookId="?">?</a></li>', notebook.NotebookId,
          notebook.Title)
    }
    body += "</ul>";
...
Notebook.changeCurNotebookTitle = function (title, isStarred, subTitle, isTag,
    isSearch) {
  var me = this;
...
Notebook.isCurNotebook = function (notebookId) {
  return $(
          tt('#notebookList [notebookId="?"], #shareNotebooks [notebookId="?"]',
              notebookId, notebookId)).attr("class") == "active";
};
...
Notebook.changeNotebookForNewNote = function (notebookId) {
  // 废纸篓
  if (Notebook.isTrashNotebookId(notebookId) || Notebook.isAllNotebookId(
          notebookId)) {
    return;
  }
  ...
  if (Notebook.curNotebookId == notebookId) {
    ...
    $("#noteItemList .item").each(function () {
      ...
      if (note.NotebookId == notebookId) {
        if (isBlog) {
          $(this).find(".item-blog").show();
        } else {
          $(this).find(".item-blog").hide();
        }
      }
    });
  }
  ajaxPost("/notebook/setNotebook2Blog",
      {notebookId: notebookId, isBlog: isBlog}, function (ret) {
        if (ret) {
...
Notebook.addNotebook = function () {
  ...
  // 添加并修改
  self.tree.addNodes(null, {Title: "", NotebookId: getObjectId(), IsNew: true},
      true, true);
}
...
Notebook.doAddNotebook = function (notebookId, title, parentNotebookId) {
  var self = Notebook;
  Service.notebookService.addNotebook(notebookId, title, parentNotebookId,
      function (ret) {
        if (ret.NotebookId) {
...
Notebook.addChildNotebook = function (target) {
  ...
  // 添加并修改
  self.tree.addNodes(self.tree.getNodeByTId(notebookId),
      {Title: "", NotebookId: getObjectId(), IsNew: true}, false, true);
}
...
Notebook.deleteNotebook = function (target) {
  ...
  if (self.getSubNotebooks(notebookId)) {
    alert(
        'This notebook has sub notebooks, please delete sub notebooks firstly.');
    return;
  }
...
Notebook.updateSync = function (notebooks) {
  ...
  for (var i in notebooks) {
    ...
    if (me.cache[notebook.NotebookId]) {
      ...
      Notebook.setCache(notebook);
      me.tree.addNodes(me.tree.getNodeByTId(notebook.ParentNotebookId),
          {Title: notebook.Title, NotebookId: notebook.NotebookId, IsNew: true},
          true, true, false);
    }
  }
};
...
Notebook.init = function () {
  ...
  function newNotebookListMenu() {
    ...
    this.popup = function (e, target, isSearch) {
      ...
      this.menu.popup(gui.getCurrentWindow(), e.originalEvent.x,
          e.originalEvent.y);
    }
  }

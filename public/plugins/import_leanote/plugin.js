          ...
          $('#chooseLeanoteFile').click(function () {
            ...
            Api.gui.dialog.showOpenDialog(Api.gui.getCurrentWindow(),
                ...
                function (paths) {
                  ...
                  if (!importService) {
                    importService = nodeRequire(
                        './public/plugins/import_leanote/import');
                  }
                  ...
                  importService.importFromLeanote(notebookId, paths,
                      ...
                      function (ok) {
                        ...
                        setTimeout(function () {
                          $('#importLeanoteMsg .allImport').html(
                              me.getMsg('Done! %s notes imported!', n));
                        }, 500);
                      },
                      // 单个文件
                      function (ok, filename) {
                        if (ok) {
                          $('#importLeanoteMsg .curImportFile').html(
                              me.getMsg("Import file: %s Success!", filename));
                        } else {
                          $('#importLeanoteMsg .curImportFile').html(me.getMsg(
                              "Import file: %s Failure, is leanote file ?",
                              filename));
                        }
                      },
                      // 单个笔记
                      function (note) {
                        if (note) {
                          n++;
                          $('#importLeanoteMsg .curImportNote').html(
                              me.getMsg("Import: %s Success!", note.Title));
                  ...
                  function dfsListDir(path, curNotebookId) {
                    ...
                    for (var i in fileNames) {
                      ...
                      if (Api.nodeFs.statSync(curFile).isDirectory()) {
                        // create node
                        var childNotebookID = Api.notebook.addChildNotebookDir(
                            curNotebookId, fileName);
                    ...
                    importService.importFromLeanote(curNotebookId, notes,
                        ...
                        function (ok, filename) {
                          if (ok) {
                            $('#importLeanoteMsg .curImportFile').html(
                                me.getMsg("Import file: %s Success!",
                                    filename));
                          } else {
                            $('#importLeanoteMsg .curImportFile').html(
                                me.getMsg(
